...
class DoubleDQN(object):
    ...
    def choose_action(self, x):
        ...
        if np.random.uniform() < EPSILON:   # greedy
            ...
            action = action[0] if ENV_A_SHAPE == 0 else np.array(action).reshape(ENV_A_SHAPE)  # return the argmax index
        else:   # random
    ...
    def learn(self):
        ...
        if self.learn_step_counter % TARGET_REPLACE_ITER == 0:
            self.target_net.load_state_dict(self.eval_net.state_dict())  # 更新目标网络，从eval中拷贝参数
        self.learn_step_counter += 1  # 计数器

        # sample batch transitions
        sample_index = np.random.choice(MEMORY_CAPACITY, BATCH_SIZE)  # 从2000个经验中选取32个索引
        b_memory = self.memory[sample_index, :]  # 从内存中取得经验
        
        b_s = torch.FloatTensor(b_memory[:, :N_STATES])  # 采样的经验，状态
        b_a = torch.LongTensor(b_memory[:, N_STATES:N_STATES+1].astype(int))  # 采样的经验，动作
        b_r = torch.FloatTensor(b_memory[:, N_STATES+1:N_STATES+2])  # 采样的经验，奖励
